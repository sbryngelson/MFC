                ...
                do r = is3%beg, is3%end
                    do k = is2%beg, is2%end
                        ...
                        do j = 0, buff_size
                            ...
                            dpres_ds = q_prim_rs${XYZ}$_vf(j, k, r, E_idx)*fd_coef_${XYZ}$ (j, cbc_loc) + dpres_ds
                            $:GPU_LOOP(parallelism='[seq]')
                            do i = 1, advxe - E_idx
                                dadv_ds(i) = q_prim_rs${XYZ}$_vf(j, k, r, E_idx + i)*fd_coef_${XYZ}$ (j, cbc_loc) + dadv_ds(i)
                            end do
                        ...
                        if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_SLIP_WALL) .or. (cbc_loc == 1 &
                            & .and. bc${XYZ}$e == BC_CHAR_SLIP_WALL)) then
                            call s_compute_slip_wall_L(lambda, L, rho, c, dpres_ds, dvel_ds)
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_NR_SUB_BUFFER) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_NR_SUB_BUFFER)) then
                            call s_compute_nonreflecting_subsonic_buffer_L(lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, &
                                & dvel_ds, dadv_ds, dYs_ds)
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_NR_SUB_INFLOW) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_NR_SUB_INFLOW)) then
                        ...
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_NR_SUB_OUTFLOW) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_NR_SUB_OUTFLOW)) then
                        ...
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_FF_SUB_OUTFLOW) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_FF_SUB_OUTFLOW)) then
                            call s_compute_force_free_subsonic_outflow_L(lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, &
                                & dadv_ds)
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_CP_SUB_OUTFLOW) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_CP_SUB_OUTFLOW)) then
                            call s_compute_constant_pressure_subsonic_outflow_L(lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, &
                                & dvel_ds, dadv_ds)
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_SUP_INFLOW) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_SUP_INFLOW)) then
                            call s_compute_supersonic_inflow_L(L)
                        else if ((cbc_loc == -1 .and. bc${XYZ}$b == BC_CHAR_SUP_OUTFLOW) .or. (cbc_loc == 1 &
                                 & .and. bc${XYZ}$e == BC_CHAR_SUP_OUTFLOW)) then
