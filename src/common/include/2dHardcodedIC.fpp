...
#:def Hardcoded2D()
    ...
    case (200)  ! Two-fluid cubic interface
        if (y%cc(j) <= (-x%cc(i)**3 + 1)**(1._wp/3._wp)) then
            ! Volume Fractions
    ...
    case (202)  ! Gresho vortex (Gouasmi et al 2022 JCP)
        r = ((x%cc(i) - 0.5_wp)**2 + (y%cc(j) - 0.5_wp)**2)**0.5_wp
        rmax = 0.2_wp
        ...
        if (r < rmax) then
            q_prim_vf(eqn_idx%mom%beg)%sf(i, j, 0) = -(y%cc(j) - 0.5_wp)*umax/rmax
            q_prim_vf(eqn_idx%mom%end)%sf(i, j, 0) = (x%cc(i) - 0.5_wp)*umax/rmax
            q_prim_vf(eqn_idx%E)%sf(i, j, 0) = p0 + umax**2*((r/rmax)**2._wp/2._wp)
        else if (r < 2*rmax) then
            q_prim_vf(eqn_idx%mom%beg)%sf(i, j, 0) = -((y%cc(j) - 0.5_wp)/r)*umax*(2._wp - r/rmax)
            q_prim_vf(eqn_idx%mom%end)%sf(i, j, 0) = ((x%cc(i) - 0.5_wp)/r)*umax*(2._wp - r/rmax)
    ...
    case (203)  ! Gresho vortex (Gouasmi et al 2022 JCP) with density correction
        r = ((x%cc(i) - 0.5_wp)**2._wp + (y%cc(j) - 0.5_wp)**2)**0.5_wp
        rmax = 0.2_wp
    ...
    case (204)  ! Rayleigh-Taylor instability
        ...
        intH = amp*sin(2._wp*pi*x%cc(i)/lam - pi/2._wp) + h

        alph = 0.5_wp*(1._wp + tanh((y%cc(j) - intH)/2.5e-3_wp))
        ...
        if (y%cc(j) > intH) then
            ...
            q_prim_vf(eqn_idx%E)%sf(i, j, 0) = pref + rhoH*9.81_wp*(1.2_wp - y%cc(j))
        else
            q_prim_vf(eqn_idx%adv%beg)%sf(i, j, 0) = alph
            q_prim_vf(eqn_idx%adv%end)%sf(i, j, 0) = 1._wp - alph
            q_prim_vf(eqn_idx%cont%beg)%sf(i, j, 0) = alph*rhoH
            ...
            q_prim_vf(eqn_idx%E)%sf(i, j, 0) = pInt + rhoL*9.81_wp*(intH - y%cc(j))
        end if
    case (205)  ! 2D lung wave interaction problem
        ...
        intH = amp*sin(2*pi*x%cc(i)/lam - pi/2) + h
    ...
    case (206)  ! 2D lung wave interaction problem - horizontal domain
        ...
        intL = amp*sin(2*pi*y%cc(j)/lam - pi/2) + h

        if (x%cc(i) > intL) then  ! this is the liquid
            q_prim_vf(eqn_idx%cont%beg)%sf(i, j, 0) = patch_icpp(1)%alpha_rho(1)
            q_prim_vf(eqn_idx%cont%end)%sf(i, j, 0) = patch_icpp(1)%alpha_rho(2)
            q_prim_vf(eqn_idx%E)%sf(i, j, 0) = patch_icpp(1)%pres
            q_prim_vf(eqn_idx%adv%beg)%sf(i, j, 0) = patch_icpp(1)%alpha(1)
    ...
    case (207)  ! Kelvin Helmholtz Instability
        ...
        gauss1 = exp(-(y%cc(j) - 0.75_wp)**2/(2.0_wp*sigma**2))
        gauss2 = exp(-(y%cc(j) - 0.25_wp)**2/(2.0_wp*sigma**2))
        q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, 0) = 0.1_wp*sin(4.0_wp*pi*x%cc(i))*(gauss1 + gauss2)
    ...
    case (250)  ! MHD Orszag-Tang vortex
        ...
        q_prim_vf(eqn_idx%mom%beg)%sf(i, j, 0) = -sin(2._wp*pi*y%cc(j))
        q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, 0) = sin(2._wp*pi*x%cc(i))

        q_prim_vf(eqn_idx%B%beg)%sf(i, j, 0) = -sin(2._wp*pi*y%cc(j))/sqrt(4._wp*pi)
        q_prim_vf(eqn_idx%B%beg + 1)%sf(i, j, 0) = sin(4._wp*pi*x%cc(i))/sqrt(4._wp*pi)
    case (251)  ! RMHD Cylindrical Blast Wave [Mignone, 2006: Section 4.3.1]
        if (x%cc(i)**2 + y%cc(j)**2 < 0.08_wp**2) then
            q_prim_vf(eqn_idx%cont%beg)%sf(i, j, 0) = 0.01
            q_prim_vf(eqn_idx%E)%sf(i, j, 0) = 1.0
        else if (x%cc(i)**2 + y%cc(j)**2 <= 1._wp**2) then
            ! Linear interpolation between r=0.08 and r=1.0
            factor = (1.0_wp - sqrt(x%cc(i)**2 + y%cc(j)**2))/(1.0_wp - 0.08_wp)
    ...
    case (252)  ! 2D MHD Rotor Problem
        ...
        r_sq = (x%cc(i) - 0.5_wp)**2 + (y%cc(j) - 0.5_wp)**2
        ...
        if (r_sq <= 0.1**2) then
            ...
            q_prim_vf(eqn_idx%mom%beg)%sf(i, j, 0) = -20._wp*(y%cc(j) - 0.5_wp)
            q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, 0) = 20._wp*(x%cc(i) - 0.5_wp)
            ...
            q_prim_vf(eqn_idx%mom%beg)%sf(i, j, 0) = -(2._wp/sqrt(r_sq))*(y%cc(j) - 0.5_wp)*(0.115_wp - sqrt(r_sq))/(0.015_wp)
            q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, 0) = (2._wp/sqrt(r_sq))*(x%cc(i) - 0.5_wp)*(0.115_wp - sqrt(r_sq))/(0.015_wp)
        end if
    case (253)  ! MHD Smooth Magnetic Vortex
        ...
        ! velocity
        q_prim_vf(eqn_idx%mom%beg)%sf(i, j, 0) = 1._wp - (y%cc(j)*exp(1 - (x%cc(i)**2 + y%cc(j)**2))/(2.*pi))
        q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, 0) = 1._wp + (x%cc(i)*exp(1 - (x%cc(i)**2 + y%cc(j)**2))/(2.*pi))

        ! magnetic field
        q_prim_vf(eqn_idx%B%beg)%sf(i, j, 0) = -y%cc(j)*exp(1 - (x%cc(i)**2 + y%cc(j)**2))/(2.*pi)
        q_prim_vf(eqn_idx%B%beg + 1)%sf(i, j, 0) = x%cc(i)*exp(1 - (x%cc(i)**2 + y%cc(j)**2))/(2.*pi)

        ! pressure
        q_prim_vf(eqn_idx%E)%sf(i, j, &
                  & 0) = 1._wp + (1 - 2._wp*(x%cc(i)**2 + y%cc(j)**2))*exp(1 - (x%cc(i)**2 + y%cc(j)**2))/((2._wp*pi)**3)
    case (260)  ! Gaussian Divergence Pulse
        ...
        ! B-field
        q_prim_vf(eqn_idx%B%beg)%sf(i, j, 0) = 1._wp + C_mhd*erf((x%cc(i) - 0.5_wp)/sigma)
    case (261)  ! Blob
        r0 = 1._wp/sqrt(8._wp)
        r2 = x%cc(i)**2 + y%cc(j)**2
        r = sqrt(r2)
        alpha = r/r0
        if (alpha < 1) then
    ...
    case (280)  ! Isentropic vortex
        ...
        if (patch_id == 1) then
            q_prim_vf(eqn_idx%E)%sf(i, j, &
                      & 0) = 1.0*(1.0 - (1.0/1.0)*(5.0/(2.0*pi))*(5.0/(8.0*1.0*(1.4 + 1.0)*pi))*exp(2.0*1.0*(1.0 - (x%cc(i) &
                      & - patch_icpp(1)%x_centroid)**2.0 - (y%cc(j) - patch_icpp(1)%y_centroid)**2.0)))**(1.4 + 1.0)
            q_prim_vf(eqn_idx%cont%beg + 0)%sf(i, j, &
                      & 0) = 1.0*(1.0 - (1.0/1.0)*(5.0/(2.0*pi))*(5.0/(8.0*1.0*(1.4 + 1.0)*pi))*exp(2.0*1.0*(1.0 - (x%cc(i) &
                      & - patch_icpp(1)%x_centroid)**2.0 - (y%cc(j) - patch_icpp(1)%y_centroid)**2.0)))**1.4
            q_prim_vf(eqn_idx%mom%beg + 0)%sf(i, j, &
                      & 0) = patch_icpp(1)%vel(1) + (y%cc(j) - patch_icpp(1)%y_centroid)*(5.0/(2.0*pi))*exp(1.0*(1.0 - (x%cc(i) &
                      & - patch_icpp(1)%x_centroid)**2.0 - (y%cc(j) - patch_icpp(1)%y_centroid)**2.0))
            q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, &
                      & 0) = patch_icpp(1)%vel(2) - (x%cc(i) - patch_icpp(1)%x_centroid)*(5.0/(2.0*pi))*exp(1.0*(1.0 - (x%cc(i) &
                      & - patch_icpp(1)%x_centroid)**2.0 - (y%cc(j) - patch_icpp(1)%y_centroid)**2.0))
        end if
    case (281)  ! Acoustic pulse
        ...
        if (patch_id == 2) then
            q_prim_vf(eqn_idx%E)%sf(i, j, &
                      & 0) = 101325*(1 - 0.5*(1.4 - 1)*(0.4)**2*exp(0.5*(1 - sqrt(x%cc(i)**2 + y%cc(j)**2))))**(1.4/(1.4 - 1))
            q_prim_vf(eqn_idx%cont%beg + 0)%sf(i, j, &
                      & 0) = 1*(1 - 0.5*(1.4 - 1)*(0.4)**2*exp(0.5*(1 - sqrt(x%cc(i)**2 + y%cc(j)**2))))**(1/(1.4 - 1))
        end if
    case (282)  ! Zero-circulation vortex
        ...
        if (patch_id == 2) then
            q_prim_vf(eqn_idx%E)%sf(i, j, &
                      & 0) = 101325*(1 - 0.5*(1.4 - 1)*(0.1/0.3)**2*exp(0.5*(1 - sqrt(x%cc(i)**2 + y%cc(j)**2))))**(1.4/(1.4 - 1))
            q_prim_vf(eqn_idx%cont%beg + 0)%sf(i, j, &
                      & 0) = 1*(1 - 0.5*(1.4 - 1)*(0.1/0.3)**2*exp(0.5*(1 - sqrt(x%cc(i)**2 + y%cc(j)**2))))**(1/(1.4 - 1))
            q_prim_vf(eqn_idx%mom%beg + 0)%sf(i, j, &
                      & 0) = 112.99092883944267*(1 - (0.1/0.3))*y%cc(j)*exp(0.5*(1 - sqrt(x%cc(i)**2 + y%cc(j)**2)))
            q_prim_vf(eqn_idx%mom%beg + 1)%sf(i, j, &
                      & 0) = 112.99092883944267*((0.1/0.3))*x%cc(i)*exp(0.5*(1 - sqrt(x%cc(i)**2 + y%cc(j)**2)))
        end if
    case (283)  ! Isentropic vortex: conserved-variable GL cell averages (3-pt tensor product)
        ...
        if (patch_id == 1) then
            ...
            do igq = 1, 3
                do jgq = 1, 3
                    xq = x%cc(i) + gauss_xi(igq)*(x%cb(i) - x%cb(i - 1))*0.5_wp
                    yq = y%cc(j) + gauss_xi(jgq)*(y%cb(j) - y%cb(j - 1))*0.5_wp
    ...
    case (291)  ! Isothermal Flat Plate
        ...
        bottom_blend_u = tanh(y%cc(j)/delta_shear)
        bottom_blend_T = tanh(y%cc(j)/delta_th)
