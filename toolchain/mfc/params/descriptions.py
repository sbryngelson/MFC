...
DESCRIPTIONS = {
    ...
    "amr": "(Experimental) Block-structured AMR: 2:1 refined level-1 block with gradient-based dynamic regrid, dt/2 subcycling, and conservative coupling with refluxing.",
    ...
    "amr_lb_block_cost": "Load balance: fixed cost charged to every block, in units of the mean block weight, added to its cell count before the per-level SFC cut (0 = cells only)",
    ...
    "l0_rebalance_interval": "L0-as-blocks spike: steps between measured-cost rebalance events that migrate tiles to level load (0 = off)",
...
PATTERNS = [
    ...
    (r"lag_params%write_void_evol", "Write void fraction evolution data"),
