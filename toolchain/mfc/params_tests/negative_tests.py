    ...
    tests.extend(
        [
            ConstraintTest(
                ...
                line_number=0,
                message="bc%x%beg must be set",
                condition="bc%x%beg is None",
                test_params={**BASE_CASE, "bc%x%beg": None},
            ),
            ConstraintTest(
                method="check_boundary_conditions",
                line_number=0,
                message="bc%x%end must be set",
                condition="bc%x%end is None",
                test_params={**BASE_CASE, "bc%x%end": None},
            ),
        ]
    )
