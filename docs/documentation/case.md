...
| `amr_lb_block_cost`    | Real    | Load balance: fixed cost charged to every block, in units of the mean block weight, added to its cells before the per-level SFC cut (default 0 = cells only). |
