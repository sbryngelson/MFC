...
parser.add_argument("--ar", type=float, default=0.0, help="Ar dilution per O2 in the bubble (>0 makes the bubble denser).")
...
# Inert N2 ambient and a light H2/O2 bubble (much lower molecular weight) at the same p, T.
ambient = ct.Solution(ctfile)
ambient.TPX = args.tamb, Pamb, "N2:1"
bubble = ct.Solution(ctfile)
bubble.TPX = args.tamb, Pamb, f"H2:2,O2:1,AR:{args.ar}" if args.ar > 0 else "H2:2,O2:1"
...
Nx = 3 * Ny
dx = Lx / Nx
R = 0.007  # bubble radius (~0.47 Ly diameter -> prominent, room to roll up)
x_bub = 0.30 * Lx  # upstream so the vortex has room to develop before exiting
...
case = {
    ...
    "chem_params%reaction_substeps": 12,
    "cantera_file": ctfile,
    "format": "silo",
    "precision": "double",
    "prim_vars_wrt": "T",
    ...
    # Patch 3: dense, reactive H2/O2/Ar bubble at rest
