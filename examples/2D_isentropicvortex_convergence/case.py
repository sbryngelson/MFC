#!/usr/bin/env python3
# 2D isentropic vortex convergence case.
# Uses hcid=283: same vortex as hcid=280 but the IC is the Gauss-Legendre cell
# average (3-pt tensor product), eliminating the O(h^2) point-value error that
# would mask high-order convergence.  With periodic BCs and a stationary vortex,
# L2(rho(T) - rho(0)) = O(h^p), the scheme's dissipation error.
import argparse
import json
import math
...
# Max wave speed: c_sound at ambient + max rotational velocity (at r=1)
c_max = math.sqrt(gamma) + 5.0 / (2.0 * math.pi)
dt = 0.4 * dx / c_max
T_end = 2.0
